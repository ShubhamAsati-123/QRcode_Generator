  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-black">
      <div className="h-128 w-128 bg-slate-500 p-5 text-white">
        <h2 className="text-5xl text-center">QR Code Generator</h2>
        <form
        ...
        <div className="flex justify-center items-center flex-col">
          {qrCodeValue && (
            <div
              ...
              ref={ref}
            >
              <QRCode size={192} value={qrCodeValue} viewBox={`0 0 256 256`} className="border-8 rounded-xl"/>
              <h1 className="text-3xl bg-gray-400 text-black rounded-lg m-5 px-5 py-2">
                Scan Me
              </h1>
            </div>
          )}
          <button onClick={onButtonClick}>Download QR Code</button>
        </div>
      </div>
    </div>
  );
